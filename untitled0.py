...
class App extends Component {
    ...
    addPosts(){
        ...
        return [...Array(Math.round(this.state.res.length-1)/noFromEach).keys()].map(
            item=>(
                <div className="row">
                    <div className="col-lg-8 blog__content">
                        <section className="section tab-post mb-16">
                            <div className="title-wrap title-wrap--line">
                                <h3 className="section-title">{this.state.res.slice(1)[item*noFromEach].name}</h3>
                            </div>
                            <div className="tabs__content tabs__content-trigger tab-post__tabs-content">
                                <div className="tabs__content-pane tabs__content-pane--active" id="tab-all">
                                    <div className="row card-row">
                                        {console.log("WatchTower", this.state.res.slice(1).slice(item*noFromEach, noFromEach*(item+1)))}
                                        {this.display(this.state.res.slice(1).slice(item*noFromEach, noFromEach*(item+1)))}
                                    </div>
                                </div>
                            </div>
                        </section>
            ...
            return (<div className="col-md-6">
                        <article className="entry card">
                          ...
                          <div className="entry__body card__body">
                            ...
                            <div className="entry__excerpt" dangerouslySetInnerHTML={{__html: item.title}}>
                            </div>
                          </div>
                        </article>
                </div>)
        ...
        return(
            <article className="entry card featured-posts-grid__entry">
               <div className="entry__img-holder card__img-holder">
                ...
                <div className="entry__body card__body">   
                  <h2 className="entry__title">
                    <a href="single-post.html">{this.latest.title}</a>
                  </h2>
                  <ul className="entry__meta">
                    <li className="entry__meta-author">
                      <span>by</span>&nbsp;
                      <a href="#">{this.latest.name}</a>
                    </li>
                    <li className="entry__meta-date">
                      {this.created_at}
                    </li>
